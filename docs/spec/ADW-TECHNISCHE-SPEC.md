...
- **Architectural principle:** "Control flow is code, not prompt." Loops, Gates, merges, Dispatch, Triage, limits, CI polling = deterministic Python (0 tokens). Agents deliver judgment exclusively — behind two narrow interfaces (`AgentRunner`, `CodexClient`) that can be replaced by mocks.
- **Dual authoring in phases 1–2:** Spec and plan are each drafted **twice in parallel and independently** — once by a Claude agent (Opus), once by Codex (`author`) — and then merged by a synthesis agent (Fable) into ONE best-of artifact plus a summary for the approval gate.
...
| `adw/codex.py`     | Codex CLI as a read-only subprocess with isolated `CODEX_HOME`; builds review AND authoring prompts (schema resp. marker protocol embedded), parses strictly | `CodexRunner.review(kind, content_refs, cwd)`, `CodexRunner.author(kind, task, cwd)`, `CodexClient` protocol                                    |
...
Each of the two phases runs a **draft stage** first (`_draft_stage`): the Claude draft author (`spec_agent`/`plan_agent`, Opus) and `ctx.codex.author(kind, …)` produce their drafts **at the same time** (`ThreadPoolExecutor`, 2 workers). The Claude author writes into the checkout as usual; the orchestrator copies the result to `.adw/runs/<id>/drafts/<name>.claude.<ext>` (atomically via tmp + `os.replace`) and resets the checkout afterwards. Codex delivers its content read-only and lands as `<name>.codex.<ext>`: because the read-only sandbox may not write, `author()` demands the file contents in marker blocks (`===BEGIN <name> <nonce>===` … `===END …===`) with a per-call nonce and parses them strictly forward from the last begin marker of the first name — a mere prompt echo yields empty blocks and thus a `CodexAuthorError` (subclass of `CodexError`), and a spec that itself documents this protocol does not terminate its own block. Idempotency runs over the FILES, not the state: an existing, non-empty draft skips its author, a `<kind>.codex.FAILED` marker prevents another Codex attempt. Crash leftovers of the phase in the checkout (artifacts *and* summary) are cleaned in the main thread **before** an author runs — a leftover must neither pass as a fresh draft nor appear to the parallel-reading Codex as an interim state. A Codex failure only **degrades** (warning + marker, single-source synthesis); a missing, empty or unchanged Claude draft escalates.

The **synthesis agent** (`spec_synthesis`/`plan_synthesis`, Fable) is then the FIRST run of the authoring loop. Its task names `.adw/issue.md`, both draft paths and the target artifacts including the summary; if the Codex draft is missing, the task carries the single-source note instead of a dead path. The **summary** (`spec-summary.md`/`plan-summary.md`) runs along as a loop artifact: it must exist (missing OR empty escalates — nobody reviews it, so a whitespace result would only surface to the human at the gate), the fix tasks carry the instruction to keep it current, and it is archived like the artifacts. It is `protected` in the plan phase like `spec.md`, is **not** in `_seed_artifacts` (no build lane builds against it) and is **not** a Codex review reference. The CLI gate messages name the summary first — it lies in the ignored run folder and would otherwise be invisible.
...
2.  **RED stage** (`_confirm_red`, only with ≥ 1 Gate `tdd: true` and only in the initial build — no `pending_task`, `red_confirmed` false, `gate_iterations == 0`): a test-only agent run with the same invariants as every build run, then `_run_lane_gates` over **exactly the marked Gates**. At least one red = proof: `red_confirmed`, the changed paths as `red_test_paths` and the implementation task as `pending_task` are persisted in **one** save, so the following loop resumes the same session with the shortened red Gate output. All green → escalation (the tests do not cover the required behavior), no retry loop.
3.  **Loop:** limit check → checkpoint (HEAD as `expected_head`, iteration++) → build agent (Opus, session resume, neighboring Lanes invisible via deny rule) → invariant checks (no agent commit, correct branch) → artifact restore → Gates.
4.  Gates red: persist error output + Circuit-Breaker baseline as `pending_task`/`last_failures`, back to 3. Gates green: `red_test_paths` still present (otherwise escalation), then persist the `gates_passed` evidence **with tree hash**, then the orchestrator commits.

The RED proof belongs to the orchestrator, not to the agent's claim: what it proves is that the marked Gates were red before the implementation run — "tests only" is an instruction to the agent, since no stack-neutral rule tells a test path from a production path. It is protected against forgery on both sides: a test-only pass that leaves the Worktree unchanged or **deletes** files proves nothing (both checks also run in the resume path, after the artifact restore), and green Gates count only while the tests that proved RED are still in place. The RED check runs **no** Gate iteration — the budget belongs to the implementation. Fix dispatches from phases 4–7 (`pending_task` set) and Lanes without a marked Gate keep the single-stage behavior byte for byte.
...
|------------------|------------------------------|-----------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------|
| `spec_agent`     | Opus 4.8 (`claude-opus-4-8`) | Read/Grep/Glob + Write **only** `.adw/spec.md`                        | Draft author of the specification following a fixed template; never implements                                    |
| `plan_agent`     | Opus 4.8                     | Read/Grep/Glob + Write **only** `.adw/plan.md` + `.adw/contract.yaml` | Draft author of plan (workstreams) + interface contract; never implements                                         |
| `spec_synthesis` | Fable 5 (`claude-fable-5`)   | Read/Grep/Glob + Write **only** `.adw/spec.md` + `.adw/spec-summary.md` | Best-of merge of the Claude and Codex spec drafts + summary for the approval gate                               |
| `plan_synthesis` | Fable 5                      | Read/Grep/Glob + Write **only** `.adw/plan.md`, `.adw/contract.yaml` + `.adw/plan-summary.md` | Best-of merge of both plan/contract drafts + summary                       |
...
**Model economics:** Opus writes the authoring drafts and builds — the draft is the deep-thinking part; Fable (broad, cheaper) merges the two drafts and does the final review; Sonnet 5 handles the assembly-line Triage. The content rules of the draft author and the synthesis are **one shared constant each** (`_SPEC_CONTENT_RULES`/`_PLAN_CONTENT_RULES`): if they drifted apart, the synthesis would undo the author's scope counterweight. Merge and summary rules (`_BEST_OF_MERGE_RULES`, `_SUMMARY_FORMAT_RULES`) are shared by both synthesis agents — a summary structured differently per phase would just be noise at the approval gate. The path rules are **artifact-exact** (e.g. `Write(.adw/spec.md)` instead of `Write(.adw/**)`) — a blanket `.adw/**` would otherwise also make `.adw/runs/<id>/state.json` writable; the synthesis agents' write rules therefore explicitly include their summary file. In addition, the run directories are deny-listed for all agents, and parallel Lanes cannot see each other via `deny_read_paths`.
...
| `session_id` of the test-only pass                                                     | Lane     | Crash between the test-only pass and the RED check: the checkpointed session marks the pass as done, so a resume repeats only the check                      |
| `red_confirmed` + `red_test_paths`                                                     | Lane     | Crash between the RED proof and the implementation run (both are written in one save with `pending_task`); the paths bind the proof to the tests that delivered it |
...
|-------------------------------|-----------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|
| Gate loop per Lane            | 10 (`MAX_GATE_ITERATIONS`) — per task, reset on a new fix task              | Circuit-Breaker `check_progress`: exactly the same failure/Finding set as in the previous round → immediate escalation instead of exhausting the limit; the RED check consumes no iteration |
...
| RED stage per Lane            | exactly 1 pass (no retry loop)                                              | No Circuit-Breaker: green marked Gates after the test-only pass → escalation instead of a test-improvement loop                                        |
...
| **codex** (CLI, verified with 0.144.0) | Independent reviews (spec/plan/code) + second draft author of spec/plan | read-only sandbox, `mcp_servers={}`, isolated `CODEX_HOME`, 900 s timeout, strict parser (Findings JSON resp. marker blocks with a per-call nonce) |
...
- **Acceptance level:** `tests/test_e2e_dry_run.py` maps the DoD criteria from `docs/SPEC.md` §8 — complete CLI dry runs (single + parallel), Gate fail→same-session fix, approval gate, Triage paths, crash resume, dual authoring, and both RED-gate paths (with and without a `tdd` Gate).
...
| In the draft stage the Codex author reads the **same checkout** as the parallel Claude author and could theoretically see a half-written artifact. | Deliberate: worktree isolation would deprive the Codex draft of the phase inputs that only exist in the checkout (`.adw/spec.md`). The Claude draft is validated afterwards (non-empty, changed) and the artifacts are reset before the synthesis. |
...
| The RED proof hangs on the test **paths**, not on their content — an implementation pass could water a test down in place; the paths themselves are not classified as tests either, so a test-only pass that also writes production code is not detected. | Deliberate: content immutability would turn the legitimate repair path of the Gate loop (the agent fixes its own broken test) into an escalation; watered-down tests are caught by the downstream Codex code review. There is also no green baseline run before the test-only pass — a Gate that is already red on HEAD (missing tool, timeout) counts as RED, but that is a project defect the loop surfaces right afterwards. A crash **before** the session checkpoint of the test-only pass repeats that agent run once (the Worktree changes are kept, so the second pass sees them). |
...
| **AgentRunner / CodexClient**         | The two protocols (PEP 544) behind which all agent calls live; `CodexClient` covers both Codex roles (`review` + `author`). Production and mock implementations are interchangeable. |
| **Draft stage (`_draft_stage`)**      | Phases 1–2: Claude author and Codex write their drafts in parallel into `.adw/runs/<id>/drafts/`; idempotent over the draft files, Codex failure degrades.             |
| **Synthesis (`spec_/plan_synthesis`)**| The agent that merges both drafts into ONE best-of artifact and writes the summary for the approval gate; it is the first run of the authoring loop.                   |
...
| **RED stage (`_confirm_red`)**        | Initial build of a Lane with a `tdd: true` Gate: test-only agent pass, then the orchestrator proves exactly the marked Gates red — `red_confirmed` + `red_test_paths` as the persisted evidence. |
