...
- **Architekturprinzip:** „Kontrollfluss ist Code, nicht Prompt." Loops, Gates, Merges, Dispatch, Triage, Limits, CI-Polling = deterministisches Python (0 Tokens). Agenten liefern ausschließlich Urteilsvermögen — hinter zwei schmalen Interfaces (`AgentRunner`, `CodexReviewer`), die per Mock ersetzbar sind.
...
| `adw/codex.py`     | Codex-CLI als read-only-Subprocess mit isoliertem `CODEX_HOME`; baut Review-Prompts (Schema eingebettet), parst strikt                                                 | `CodexRunner.review(kind, content_refs, cwd)`, `CodexReviewer`-Protokoll                                                                        |
...
2.  **Loop:** Limit-Check → Checkpoint (HEAD als `expected_head`, Iteration++) → Build-Agent (Opus, Session-Resume, Nachbar-Lanes per Deny-Regel unsichtbar) → Invarianten-Checks (kein Agent-Commit, richtiger Branch) → Artefakt-Restore → Gates.
3.  Gates rot: Fehlerausgabe + Circuit-Breaker-Basis als `pending_task`/`last_failures` persistieren, zurück zu 2. Gates grün: `gates_passed`-Beweis **mit Baum-Hash** persistieren, dann committet der Orchestrator.
...
|------------------|------------------------------|----------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|
| `spec_agent`     | Fable 5 (`claude-fable-5`)   | Read/Grep/Glob + Write **nur** `.adw/spec.md`                        | Spezifikation nach fester Vorlage; implementiert nie                                                |
| `plan_agent`     | Fable 5                      | Read/Grep/Glob + Write **nur** `.adw/plan.md` + `.adw/contract.yaml` | Plan (Workstreams) + Schnittstellen-Kontrakt; implementiert nie                                     |
...
**Modell-Ökonomie:** das teuerste Urteilsvermögen (Fable) nur an den Hebelpunkten Spec/Plan/finaler Review; Opus baut; Sonnet 5 erledigt die Fließband-Triage. Die Pfad-Regeln sind **artefakt-exakt** (z. B. `Write(.adw/spec.md)` statt `Write(.adw/**)`) — ein pauschales `.adw/**` würde sonst auch `.adw/runs/<id>/state.json` beschreibbar machen. Zusätzlich sind die Run-Verzeichnisse für alle Agents deny-gelistet und parallele Lanes sehen einander per `deny_read_paths` nicht.
...
| **codex** (CLI, geprüft mit 0.144.0) | Unabhängige Reviews (spec/plan/code)                         | read-only-Sandbox, `mcp_servers={}`, isoliertes `CODEX_HOME`, 900-s-Timeout, strikter Parser |
...
| **AgentRunner / CodexReviewer**       | Die zwei Protokolle (PEP-544), hinter denen alle Agent-Aufrufe liegen; Produktions- und Mock-Implementierung sind austauschbar.                                     |
